...
class NetworkModule {
    ...
    @Singleton
    @Provides
    fun providesCacheDirectory(@ApplicationContext context: Context): File = File(context.cacheDir.absolutePath, context.resources.getString(
        R.string.ws_cache_directory_name))

    @Singleton
    @Provides
    fun providesCache(@ApplicationContext context: Context, cacheDirectory: File): Cache = Cache(cacheDirectory, context.resources.getString(
        R.string.ws_cache_size).toLong())
